...
class AgentPool extends Subject {
  ...
  async runTest(record) {
    ...
    const { conditionals } = this.options;

    if (conditionals && conditionals.some(entry => test.file.startsWith(entry))) {
      this.next(agent);

      test.rawResult = {
        error: {
          name: 'No execution',
          message: 'Test failure for skipped execution from conditional list.',
        },
        stdout: '',
        stderr: '',
      };
    ...
    const result = agent.evalScript(test, {async, module});
    let stopPromise;
