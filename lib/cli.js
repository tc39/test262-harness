...
const yargv = yargs
  ...
  .describe('conditional', 'Use a conditional list tests to be filtered for execution. The parameter should provide a text file with a list of files and/or folders, separated by lines. Any file with a matching in this list will be flagged as a failure.')
  .alias('conditional', 'c')
  .nargs('conditional', 1)
  ...
  .example('test262-harness --hostType xs --hostPath path/to/host --conditional known_failures.txt -- path/to/test262/**/*.js')
  .fail((msg, err) => {
    if (err) {
